"""Transient wizard for sending fault notification emails."""
...
class VehicleFaultWizard(models.TransientModel):
    """Wizard to notify the fleet manager when a vehicle is marked faulty.

    Collects the manager, email address, and fault comment, then sends
    the configured mail template and updates the inspection state.
    """
    ...
    def action_send_fault_email(self):
        """Send the fault mail template and mark the inspection as faulty.
