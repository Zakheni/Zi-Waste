"""Inspection line model for checklist rows and fault photos."""
...
class VehicleInspectionLine(models.Model):
    """Single row on a vehicle inspection checklist.

    Lines may be section headers (display_type line_section) or item rows
    with a clear/fault result, optional notes, and attached photos.
    """
    ...
    def _compute_photo_count(self):
        """Count the number of photos attached to each line."""
        for rec in self:
    ...
    def _check_required_photos(self):
        """Ensure fault lines with require_photo have at least one photo.

        Raises:
            ValidationError: When a required photo is missing on a fault line.
        """
        for inspection in self:
    ...
    def action_open_photos(self):
        """Open a popup listing photos linked to this inspection line.

        Returns:
            dict: Window action for vehicle.inspection.image records.
        """
        self.ensure_one()
    ...
    def action_open_photo_popup(self):
        """Open the line form in a popup to upload fault photos.

        Returns:
            dict: Window action for this inspection line form view.
        """
        self.ensure_one()
