"""Sync history log, outbound queue, and CRUD hooks pushing Odoo changes to Sage."""
...
class PastelSyncLog(models.Model):
    """Persist import counts per entity kind for audit and reporting."""
...
class PastelSyncQueue(models.Model):
    """Queue of create/write/unlink operations to push to the Pastel bridge."""
    ...
    def _push_enabled(self):
        """Return whether outbound push to Sage is enabled in connector settings."""
    ...
    def process_queue(self, limit=50):
        """Process pending queue entries by calling the appropriate bridge endpoints."""
...
class ResPartner(models.Model):
    """Enqueue partner create/write/unlink events for Sage customer/supplier sync."""

    _inherit = "res.partner"

    def _pastel_role(self):
        """Return whether the partner acts as customer, supplier, or both."""
        self.ensure_one()
    ...
    def _to_pastel_payload(self):
        """Build JSON payload for the Pastel partner endpoints."""
        self.ensure_one()
    ...
    def create(self, vals):
        """Enqueue a create operation when push to Sage is enabled."""
    ...
    def write(self, vals):
        """Enqueue a write operation when push to Sage is enabled."""
    ...
    def unlink(self):
        """Enqueue an unlink operation when push to Sage is enabled."""
        if self.env["pastel.sync.queue"]._push_enabled():
            for rec in self:
                self.env["pastel.sync.queue"].sudo().create({
                    "model": "res.partner",
...
class ProductTemplate(models.Model):
    """Enqueue product template changes for Sage product sync."""

    _inherit = "product.template"

    def _to_pastel_payload(self):
        """Build JSON payload for the Pastel product endpoint."""
        self.ensure_one()
        return {
    ...
    @api.model
    def create(self, vals):
        """Enqueue product create when push to Sage is enabled."""
        rec = super().create(vals)
        if self.env["pastel.sync.queue"]._push_enabled():
            self.env["pastel.sync.queue"].sudo().create({
    ...
    def write(self, vals):
        """Enqueue product write when push to Sage is enabled."""
        res = super().write(vals)
        if self.env["pastel.sync.queue"]._push_enabled():
            for rec in self:
                self.env["pastel.sync.queue"].sudo().create({
    ...
    def unlink(self):
        """Enqueue product unlink when push to Sage is enabled."""
        if self.env["pastel.sync.queue"]._push_enabled():
            for rec in self:
                self.env["pastel.sync.queue"].sudo().create({
                    "model": "product.template",
...
class AccountMove(models.Model):
    """Enqueue customer invoice changes for Sage invoice sync."""

    _inherit = "account.move"

    def _to_pastel_payload(self):
        """Build JSON payload for the Pastel invoice endpoint."""
        self.ensure_one()
        data = {
    ...
    @api.model
    def create(self, vals):
        """Enqueue customer invoice create when push to Sage is enabled."""
        rec = super().create(vals)
    ...
    def write(self, vals):
        """Enqueue customer invoice write when push to Sage is enabled."""
        res = super().write(vals)
        if self.env["pastel.sync.queue"]._push_enabled():
    ...
    def unlink(self):
        """Enqueue customer invoice unlink when push to Sage is enabled."""
        if self.env["pastel.sync.queue"]._push_enabled():
            for rec in self.filtered(lambda m: m.move_type == "out_invoice"):
                self.env["pastel.sync.queue"].sudo().create({
                    "model": "account.move",
