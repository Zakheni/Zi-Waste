"""Website portal controllers for waste service requests and worksheets.

Exposes authenticated portal routes for customers and client agents to log
requests, view manifests, complete worksheets, manage pickup points, and
access dashboard/report exports.
"""
import json
...
def _portal_workflow_steps(state):
    """Build portal status-stepper data for a service request state."""
    if state == 'cancelled':
        return {'cancelled': True, 'steps': []}
    order = [key for key, _label in PORTAL_WORKFLOW_STATES]
    try:
        current_idx = order.index(state)
    except ValueError:
        current_idx = -1
    steps = []
    for idx, (key, label) in enumerate(PORTAL_WORKFLOW_STATES):
        if current_idx < 0:
...
class WasteClientPortal(CustomerPortal):
    """Portal controller for waste service requests and worksheets.

    Extends the standard customer portal with WMZ-specific list, detail,
    creation, worksheet, dashboard, and reporting routes. Access is scoped
    by user role (customer vs. client agent) and company.
    """

    def _base_request_domain(self, user):
        """Build the search domain for service requests visible to *user*.

        Agents see requests assigned to them; customers see requests for
        their commercial partner hierarchy within allowed states.

        :param user: Current portal user.
        :type user: res.users
        :return: Odoo domain list for ``waste.service.request`` searches.
        :rtype: list
        """
    ...
    def _prepare_home_portal_values(self, counters):
        """Add waste request counter to the portal home page.

        Also hides invoice/order/quotation counters for client agents who
        should not see standard sales portal tiles.

        :param counters: Counter names requested by the portal layout.
        :type counters: list
        :return: Template values including ``waste_request_count``.
        :rtype: dict
        """
        values = super()._prepare_home_portal_values(counters)
        ...
        values['waste_request_count'] = WasteRequest.search_count(domain)

        # Hide invoices & orders for agents
        if user.has_group('waste_management_zakheni.group_wmz_client_agent'):
            values.pop('invoice_count', None)
            values.pop('order_count', None)
            values.pop('quotation_count', None)


        return values

    def _prepare_portal_layout_values(self):
        """Add portal home summary stats for the welcome hero."""
        ...
        WasteRequest = request.env['waste.service.request'].sudo()
        domain = self._base_request_domain(user)
        values['wmz_home_request_count'] = WasteRequest.search_count(domain)
        values['wmz_home_active_count'] = WasteRequest.search_count(
            domain + [('state', 'in', ['generated', 'scheduled', 'dispatched'])]
        )
        return values

    @http.route([
        '/my/waste/requests',
        '/my/waste/requests/page/<int:page>'
    ], type='http', auth='user', website=True)
    def portal_my_waste_requests(self, page=1, search=None, **kw):
        """List the current user's waste service requests with pagination.

        Route: ``/my/waste/requests`` (and paginated variant).

        Supports optional ``state`` filter (including ``open``) and free-text
        search on name, partner, service, and container type.

        :param int page: Page number (1-based).
        :param str search: Optional search string.
        :param dict kw: Query parameters (``page``, ``state``, etc.).
        :return: Rendered list template with records and pager.
        :rtype: werkzeug.wrappers.Response
        """

        page = int(kw.get('page', page))  # ✅ FIX
    ...
    #     WasteRequest = request.env['waste.service.request'].sudo()
        ...
        if search:
            domain += [
                ...
                ('service_requested_id.name', 'ilike', search),
                ('container_type_id.name', 'ilike', search),
            ]

        total = WasteRequest.search_count(domain)

        # ✅ Clean url args (VERY IMPORTANT)
        url_args = {}
        if search:
        ...
        values = {
            'page_name': 'waste_requests',  # ✅ ADD THIS
            'wsr_records': wsr,
            'pager': pager,
            'search': search,
            'active_state': state,
        ...
        # print("TOTAL:", total)
        # print("PAGER:", pager)
        # _logger.warning(f"PAGE={page}, OFFSET={pager['offset']}")
    ...
    def portal_new_waste_request_form(self, **kwargs):
        """Render the portal form to log a new waste service request.

        Route: ``/my/waste/request/new``.

        Client agents are redirected away. Customers receive pickup points,
        company-scoped configuration options, and admin clerk contacts.

        :param dict kwargs: Unused route kwargs.
        :return: Rendered new-request form or redirect for agents.
        :rtype: werkzeug.wrappers.Response
        """
        user = request.env.user
        ...
        values = {
            ...
            "container_types": company.wmz_container_type_ids if "wmz_container_type_ids" in company._fields else [],
            "services": company.wmz_service_ids if "wmz_service_ids" in company._fields else [],
            "waste_types": company.wmz_waste_type_ids if "wmz_waste_type_ids" in company._fields else [],
            "waste_details": company.wmz_waste_details_ids if "wmz_waste_details_ids" in company._fields else [],
            "bin_types": company.wmz_bin_type_ids if "wmz_bin_type_ids" in company._fields else env[
    ...
    def portal_create_waste_request(self, **post):
        """Create a waste service request from portal form submission.

        Route: ``/my/waste/request/create`` (POST).

        Validates company-scoped selections, creates the request with
        ``from_portal=True``, and optionally emails the selected admin clerk.

        Side effects:
            - Creates a ``waste.service.request`` record.
            - May send ``mail_tmpl_service_request_portal_completion``.

        :param dict post: Submitted form fields.
        :return: Redirect to thank-you page or back to form on validation error.
        :rtype: werkzeug.wrappers.Response
        """
        ...
        # ------------------------------------------------------------
        # ✅ FORCE CLIENT (commercial partner)
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # ✅ PICKUP POINTS (single → m2m)
        # ------------------------------------------------------------
        # pickup_point_ids = []
        # if post.getlist('pickup_point_ids'):
        #     try:
        #         pickup_point_ids = [int(x) for x in post.getlist('pickup_point_ids') if x]
        #     except Exception:
        #         pickup_point_ids = []

        pickup_point_ids = []
        raw = post.get('pickup_point_ids')

        if raw:
            try:
                pickup_point_ids = [int(x) for x in raw.split(',') if x]
            except Exception:
                pickup_point_ids = []
        ...
        # ------------------------------------------------------------
        # ✅ CONTAINER TYPE (ID from form)
        # ------------------------------------------------------------
        container_type_id = int(post.get('container_type_id') or 0)

        _logger.info(
            "PORTAL CREATE | container_type_id=%s | allowed=%s",
            container_type_id,
            company.wmz_container_type_ids.ids,
        )

        if container_type_id and container_type_id not in company.wmz_container_type_ids.ids:
            return request.redirect('/my/waste/request/new?msg=invalid_container')

        # ------------------------------------------------------------
        # ✅ WASTE TYPE (ID from form)
        # ------------------------------------------------------------
        waste_type_id = int(post.get('waste_type_id') or 0)

        _logger.info(
            "PORTAL CREATE | waste_type_id=%s | allowed=%s",
            waste_type_id,
            company.wmz_waste_type_ids.ids,
        )

        if waste_type_id and waste_type_id not in company.wmz_waste_type_ids.ids:
            return request.redirect('/my/waste/request/new?msg=invalid_waste_type')

        # ------------------------------------------------------------
        # ✅ SERVICE REQUESTED (ID from form)
        # ------------------------------------------------------------
        service_requested_id = int(post.get('service_requested_id') or 0)

        _logger.info(
            "PORTAL CREATE | service_requested_id=%s | allowed=%s",
            service_requested_id,
            company.wmz_service_ids.ids,
        )

        if service_requested_id and service_requested_id not in company.wmz_service_ids.ids:
            return request.redirect('/my/waste/request/new?msg=invalid_service')

        # ------------------------------------------------------------
        # ✅ WASTE DETAILS (SAME PATTERN AS WASTE TYPE)
        # ------------------------------------------------------------
        waste_details_id = int(post.get('waste_details_id') or 0)

        _logger.info(
            "PORTAL CREATE | waste_details_id=%s | allowed=%s",
            waste_details_id,
            company.wmz_waste_details_ids.ids,
        )

        if waste_details_id and waste_details_id not in company.wmz_waste_details_ids.ids:
            return request.redirect('/my/waste/request/new?msg=invalid_waste_details')



        # ------------------------------------------------------------
        # ✅ BASE VALUES (ALL CORE FIELDS)
        # ------------------------------------------------------------
        vals = {
            ...
            'waste_details_id': waste_details_id or False,
            'service_requested_id': service_requested_id or False,
            'from_portal': True,
        }

        if waste_details_id:
            vals['waste_details_id'] = waste_details_id
        # ------------------------------------------------------------
        # ✅ BIN / TANK SIZE (DERIVED FROM CONTAINER TYPE)
        # ------------------------------------------------------------
        if container_type_id:
            ct = env['container.type'].sudo().browse(container_type_id)
            ct_name = (ct.name or '').lower()

            if 'bin' in ct_name:
                if post.get('bin_type_id'):
                    vals['bin_type_id'] = int(post.get('bin_type_id'))
                vals['tank_volume_id'] = False

            elif 'tank' in ct_name:
                if post.get('tank_volume_id'):
        ...
        _logger.info(
            "PORTAL CREATE SUCCESS | request_id=%s | company=%s",
            wsr.id,
            company.id,
        )
    ...
    def portal_waste_request_thankyou(self, wsr_id, **kw):
        """Display confirmation page after a portal request is created.

        Route: ``/my/waste/request/thankyou/<wsr_id>``.

        :param int wsr_id: ID of the newly created service request.
        :param dict kw: Unused route kwargs.
        :return: Rendered thank-you page or 404 if the request does not exist.
        :rtype: werkzeug.wrappers.Response
        """
    ...
    def portal_waste_request_detail(self, wsr_id, **kw):
        """Show a single service request and related worksheets.

        Route: ``/my/waste/request/<wsr_id>``.

        Enforces record-level access via :meth:`_base_request_domain`. Agents
        may see linked worksheets when the request is in an agent-visible state.

        :param int wsr_id: Service request database ID.
        :param dict kw: Optional query params (e.g. ``msg`` flash message).
        :return: Rendered detail template or 404 when access is denied.
        :rtype: werkzeug.wrappers.Response
        """
        WasteRequest = request.env['waste.service.request'].sudo()
        ...
        values = {
            ...
            'workflow': _portal_workflow_steps(wsr.state),
    ...
    def portal_waste_worksheet_edit(self, worksheet_id, **kw):
        """Render the portal worksheet edit form for client agents.

        Route: ``/my/waste/worksheet/<worksheet_id>/edit``.

        Loads manifest context (pickup/dropoff, bins, planned date), UoMs,
        and manager contacts for completion notification.

        :param int worksheet_id: Worksheet database ID.
        :param dict kw: Optional query params (e.g. ``msg``).
        :return: Rendered worksheet form, redirect, or 404.
        :rtype: werkzeug.wrappers.Response
        """
        user = request.env.user
    ...
    def _get_template_sudo(self, xmlid):
        """Resolve a mail template by XML ID with elevated access.

        Portal users may lack read rights on ``mail.template``; this helper
        resolves the template via ``ir.model.data`` and returns it with sudo.

        :param str xmlid: Full XML ID (e.g. ``module.template_xmlid``).
        :return: Existing ``mail.template`` record, or empty recordset.
        :rtype: mail.template
        """
    ...
    def portal_waste_worksheet_save(self, worksheet_id, **post):
        """Persist portal worksheet edits and advance workflow states.

        Route: ``/my/waste/worksheet/<worksheet_id>/save`` (POST).

        Side effects:
            - Writes field values, documents, and signatures on the worksheet.
            - Sets worksheet state to ``done`` and may set the manifest to
              ``service_delivered``.
            - Sends completion emails when states change.
            - Creates, updates, or removes worksheet photo attachments.

        :param int worksheet_id: Worksheet database ID.
        :param dict post: Submitted form fields and file uploads.
        :return: Redirect back to the edit form with success message.
        :rtype: werkzeug.wrappers.Response
        """
        user = request.env.user
    ...
    def portal_waste_dashboard(self, **kw):
        """Render the main waste portal dashboard with KPIs and reports.

        Route: ``/my/waste``.

        Aggregates request counts, chart data from worksheets, paginated
        client report rows, and recent driver trips. Supports filter query
        params for the client report section.

        :param dict kw: Filters and pagination (``date_from``, ``report_page``,
            ``trips_page``, etc.).
    ...
    def _get_allowed_client_ids(self, user):
        """Return partner IDs the portal user may access.

        Includes the user's commercial partner and all child contacts.

        :param user: Current portal user.
        :type user: res.users
        :return: List of allowed ``res.partner`` IDs.
        :rtype: list
        """
    ...
    def portal_waste_print_report(self, **kw):
        """Render an HTML waste report for printing in the browser.

        Route: ``/my/waste/report``.

        Builds client report rows (manifest / sales order / invoice), a
        requests summary, and recent driver trips using the same access rules
        as the dashboard.

        :param dict kw: Report filters (dates, manifest/SO/invoice numbers).
        :return: Rendered HTML report page.
        :rtype: werkzeug.wrappers.Response
        """
        user = request.env.user
        env = request.env(context=dict(request.env.context, tz='Africa/Johannesburg'))
    ...
    def portal_waste_report_pdf(self, **kw):
        """Download a PDF waste service report for the current customer.

        Route: ``/my/waste/report/pdf``.

        Filters manifests by commercial partner and query params, then renders
        ``action_portal_waste_pdf`` via QWeb.

        :param dict kw: Report filters (dates, manifest/SO/invoice numbers).
        :return: PDF HTTP response or redirect when no records match.
        :rtype: werkzeug.wrappers.Response
        """
        user = request.env.user
        env = request.env(context=dict(request.env.context, tz='Africa/Johannesburg'))
    ...
    def portal_waste_report_xlsx(self, **kw):
        """Download an Excel workbook with waste portal report data.

        Route: ``/my/waste/report/xlsx``.

        Produces three sheets: Client Report, Requests, and Driver Trips.

        :param dict kw: Report filters (dates, manifest/SO/invoice numbers).
        :return: XLSX file HTTP response.
        :rtype: werkzeug.wrappers.Response
        """
        import io
        import xlsxwriter
    ...
    def portal_pickup_points_by_customer(self, **kw):
        """Return pickup points for a customer as JSON (AJAX).

        Route: ``/my/waste/pickup_points`` (POST).

        Expects JSON body with ``partner_id``. Validates that the partner is
        an active company customer in the current company.

        :param dict kw: Unused route kwargs.
        :return: JSON list of ``{id, name}`` pickup point dicts.
        :rtype: werkzeug.wrappers.Response
        """
    ...
    def portal_pickup_points_create(self, **kw):
        """Create a pickup point for the logged-in customer from the portal.

        Route: ``/my/waste/pickup_points/create`` (POST).

        Side effects:
            - Creates a ``pickup.point`` linked to the user's commercial partner.
            - Writes an audit log entry on the pickup point.

        Client agents are rejected.

        :param dict kw: Unused route kwargs; payload is read from JSON body.
        :return: JSON with new point ``id`` and ``name``, or error payload.
        :rtype: werkzeug.wrappers.Response
        """
        # ✅ Read JSON body (fetch sends raw JSON)
        try:
        ...
        if not name:
            return request.make_json_response({
                'error': _('Address Name is required.')
            })

        user = request.env.user
        env = request.env

        # 🔒 FORCE client to logged-in user's commercial partner
    ...
    def portal_pickup_point_edit(self, pp_id, **kw):
        """Rename a pickup point owned by the logged-in customer.

        Route: ``/my/waste/pickup_points/<pp_id>/edit`` (POST).

        Side effects:
            - Updates the pickup point name.
            - Writes an audit log entry.

        :param int pp_id: Pickup point database ID.
        :param dict kw: Unused route kwargs; payload is JSON with ``name``.
    ...
    def portal_pickup_point_archive(self, pp_id):
        """Archive (deactivate) a pickup point owned by the logged-in customer.

        Route: ``/my/waste/pickup_points/<pp_id>/archive`` (POST).

        Side effects:
            - Sets ``active=False`` on the pickup point.
            - Writes an audit log entry.
    ...
    @http.route('/my/waste/requests/data', type='json', auth='user')
    def portal_waste_requests_data(self):
        """Return all service requests as JSON for client-side widgets.

        Route: ``/my/waste/requests/data`` (JSON-RPC).

        Note: currently searches all requests with sudo (no domain filter).

        :return: List of request summary dicts for DataTables or similar UI.
        :rtype: list
        """

        records = request.env['waste.service.request'].sudo().search([])
        ...
        for r in records:
            data.append({
                "name": r.name,
                "service": r.service_requested_id.display_name or "",
                "customer": r.partner_id.name or "",
                "container": r.container_type_id.display_name or "",
                "created_by": r.create_uid.partner_id.name,
                "updated_by": r.write_uid.partner_id.name,
                "created": r.service_request_date.strftime('%Y-%m-%d') if r.service_request_date else "",
                "updated": r.write_date.strftime('%Y-%m-%d') if r.write_date else "",
                "status": r.state_label or "",
                "id": r.id
            })
    ...
    def wmz_ping(self, **kw):
        """Health-check endpoint confirming the WMZ portal controller is loaded.

        Route: ``/wmz/ping`` (public).

        :param dict kw: Unused route kwargs.
        :return: Plain-text ``WMZ OK`` response.
        :rtype: str
        """
        return "WMZ OK"
