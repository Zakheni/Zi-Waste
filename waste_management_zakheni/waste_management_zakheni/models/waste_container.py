"""Physical waste containers (bins and tanks) and related product/SO extensions."""
from odoo import models, fields, api
from odoo.exceptions import UserError


class WasteContainer(models.Model):
    """Trackable bin or tank asset with reservation and placement state."""
    ...
    def create(self, vals):
        """Assign sequence bin number and default company on create."""
    ...
    status = fields.Selection([
        ...
        ('missing', 'Missing'),
        ('un_use', 'UnUse'),
    ], default='intact', string='Condition', tracking=True, group_expand='_read_group_expand_status')
    ...
    def _compute_status_label(self):
        ...
        for rec in self:
            rec.status_label = selection.get(rec.status, rec.status or '')

    @api.model
    def _read_group_expand_status(self, statuses, domain, order):
    ...
    def _compute_hide_container(self):
        """Show/hide tank vs bin fields based on container type."""
        for rec in self:
    ...
    def _compute_display_info(self):
        """Display bin type or tank volume as a single label."""
        for rec in self:
    ...
    def _compute_liters_remaining(self):
        """Compute remaining tank capacity after collection."""
        for rec in self:
            try:
    ...
    def _check_unique_name(self):
        """Ensure bin numbers are unique across all containers."""
        for rec in self:
            if not rec.name:
                continue
    ...
    def action_open_reserved_request(self):
        """Open the manifest this container is reserved for."""
        self.ensure_one()
        if not self.reserved_request_id:
            return False
        return {
            ...
            'view_mode': 'form',
            'res_id': self.reserved_request_id.id,
            'target': 'current',
        }


class ProductTemplate(models.Model):
    """Link waste containers to product templates for quoting."""
...
class SaleOrderLine(models.Model):
    """Expose customer and pickup point on sale order lines."""
