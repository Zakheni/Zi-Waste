"""Tank volume capacity master data for tanker and tank assets."""
from odoo import models, fields, api, _
from odoo.exceptions import UserError, AccessDenied, ValidationError


class TankVolume(models.Model):
    """Standard tank capacities (litres) used in billing and display."""
    _name = 'tank.volume'
    ...
    @api.onchange('pav_id')
    def _onchange(self):
        """Sync tank volume name from the linked attribute value."""
    ...
    def _check_unique_name(self):
        """Ensure tank volume names are unique."""
        for rec in self:
            if rec.name:
