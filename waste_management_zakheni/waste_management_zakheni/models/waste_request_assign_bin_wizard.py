"""Transient wizard to assign bins and tanks to pickup points on a manifest."""
...
class WasteAssignBinWizard(models.TransientModel):
    """Multi-step bin/tank assignment for a service request."""
    ...
    def action_confirm(self):
            ...
            for line in self.line_ids:
                ...
                if line.bin_lifted_ids:
                    line.bin_lifted_ids.write({
                        ...
                        'inUse': False,
                        'status': 'intact',  # added
                        'reserved_request_id': False,  # added
                    })
        ...
        if svc in REMOVE_FROM_CUSTOMER_SERVICES and all_lifted:
            all_lifted.write({
                ...
                'dropoff_point_id': False,
                'inUse': False,
                'status': 'intact',  # added from un_use to intact
                'reserved_request_id': False,  # added
            })

        # ----------------------------------------------------
        # 🟠 APPLY DROPPED BINS TO CUSTOMER (ALL SERVICES)
        # ----------------------------------------------------
        if all_dropped:
            for line in self.line_ids:
                pp = line.pickup_point_id or line.dropoff_point_id
        ...
        # ----------------------------------------------------
        # 🔵 SYNC TO WORKSHEET (CRITICAL FIX)
        # ----------------------------------------------------
        worksheet = self.env['waste.worksheet'].search([
            ('service_request_id', '=', req.id)
        ], limit=1)

        if worksheet:
            worksheet.write({
                'bin_lifted_ids': [(6, 0, all_lifted.ids)],
                'bin_dropped_ids': [(6, 0, all_dropped.ids)],
                'pickup_point_ids': [(6, 0, pickup_points.ids)],
                'dropoff_point_ids': [(6, 0, dropoff_points.ids)],
            })
...
class WasteAssignBinWizardLine(models.TransientModel):
    """Single pickup-point row inside the assign-bin wizard."""
