...
class ServiceRequestUser(models.Model):
    ...
    #             })
    #
    #             # 🟣 4. Handle roles SAFELY (same logic as update)
    #             managed_groups = self.env['res.groups'].search([
    #                 ('category_id.name', 'in', [
    #                     'Waste Management',
    #                     'Pastel Connector',
    #                     'Vehicle Inspection'
    ...
    #             ])
    #
    #             # New user usually has minimal groups, but keep safe logic consistent
    #             groups_to_keep = user.groups_id - managed_groups
    #
    #             new_groups = groups_to_keep | rec.role_ids
    #
    #             # Always include internal user
    #             base_group = self.env.ref('base.group_user')
    #             new_groups |= base_group
    #
    #             # ✅ Apply groups (single clean write)
    #             user.write({
    #                 'groups_id': [(6, 0, new_groups.ids)]
    #             })
    #
    #             # 🟡 5. Send invite
    #             user.sudo().with_context(create_user=True).action_reset_password()
    ...
    #                 'partner_id': partner.id,
    #                 'invite_url': invite_url,
    #                 'message': f'<strong>Invitation link:</strong><br/>'
    #                            f'<a href="{invite_url}" target="_blank">{invite_url}</a>',
    #                 'state': 'created',
    #             })
    #
    #         except Exception as e:
    ...
    def action_update_user(self):
        for rec in self:
            try:
                ...
                # ✅ Define managed groups (ONLY your custom ones)
                managed_groups = self.env['res.groups'].search([
                    ('category_id.name', 'in', [
                        'Waste Management',
                        'Pastel Connector',
                ...
                # ✅ Keep all OTHER groups (Sales, Inventory, etc.)
                groups_to_keep = user.groups_id - managed_groups

                # ✅ Add selected roles from UI
                new_groups = groups_to_keep | rec.role_ids

                # ✅ Always ensure internal user
                base_group = self.env.ref('base.group_user')
                new_groups |= base_group

                # ✅ Apply final groups (replace safely)
                user.write({
                    'groups_id': [(6, 0, new_groups.ids)]
                })
