"""Invoice fields and actions for batch payment workflow."""

from odoo import models, fields, api


class AccountMove(models.Model):
    """Extend invoices with batch payment state and register-payment helper."""
    ...
    def _compute_hide_payment_buttons(self):
        """Hide register buttons while batch is draft/validated."""
        for invoice in self:
    ...
    def _compute_is_paid_via_batch(self):
        """True when invoice was fully paid via a batch."""
        for move in self:
    ...
    def action_register_payment_batch(self):
        """Open payment registration without immediate reconciliation."""
        self.ensure_one()
